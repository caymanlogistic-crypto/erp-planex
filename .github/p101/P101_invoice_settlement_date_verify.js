...
(async () => {
  ...
  try {
    ...
    // POST the exact rendered form with its real CSRF token, but keep the
    // existing date unchanged. This validates route + CSRF + service while
    // remaining an explicit no-op in production data.
    const directResult = await form.evaluate(async formEl => {
    ...
    let saveJson = null;
    try { saveJson = JSON.parse(directResult.text); } catch (_) {}
    console.log('P101_DIRECT_POST=' + JSON.stringify(directResult));
    ok(directResult.status === 200, 'settlement date POST HTTP ' + directResult.status + ': ' + directResult.text);
    ok(saveJson && saveJson.ok === true, 'settlement date POST did not return ok=true: ' + directResult.text);
    ok(saveJson.operation_date === '2026-08-18', 'settlement date POST returned unexpected date: ' + directResult.text);
    ok(saveJson.changed === false, 'same-date production verifier must remain a no-op: ' + directResult.text);
