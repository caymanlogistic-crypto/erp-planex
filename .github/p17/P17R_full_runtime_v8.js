...
if (wrapper.split(malformedCash).length !== 2) {
  throw new Error(`P17R v8 cash-response anchor mismatch: ${wrapper.split(malformedCash).length - 1}`);
}
...
if (wrapper.split(compileTail).length !== 2) {
  throw new Error(`P17R v8 compile-tail anchor mismatch: ${wrapper.split(compileTail).length - 1}`);
}
...
const filename = path.join(__dirname, 'P17R_full_runtime_v8.wrapper.compiled.js');
const runtimeModule = new Module(filename, module);
runtimeModule.filename = filename;
runtimeModule.paths = module.paths;
