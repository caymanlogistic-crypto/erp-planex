...
(async () => {
  ...
  try {
    ...
    const expectedBankHeaders = ['Дата','Счёт','Контрагент','ИНН','Назначение','Дебет','Кредит'];
    ok(JSON.stringify(headers) === JSON.stringify(expectedBankHeaders), 'current bank headers ' + JSON.stringify(headers));
    for (const legacyLabel of ['Разнести','ЦФУ','Статья ДДС','Статус']) ok(!headers.includes(legacyLabel), 'legacy bank column removed ' + legacyLabel);
    ...
    for (const name of ['date_from','date_to']) ok(await page.locator(`form.bank-controls-filter [name="${name}"]`).count() === 1, 'filter ' + name);
    ok(await page.locator('.bank-controls-right .toolbar-search').count() === 1, 'bank local search');
    ok(await page.locator('form.bank-controls-filter [name="q"],form.bank-controls-filter [name="classification_status"]').count() === 0, 'obsolete bank server filters removed');
    ...
    await firstRow.click(); await page.waitForTimeout(250);
    ...
    await detail.waitFor({ state: 'visible', timeout: 10000 }); await page.waitForTimeout(400);
    ...
    if (!detailText.includes('внутренний перевод')) {
      ...
      const bankEmployeeOptions = detail.locator('select[name="employee_user_id"] option');
      if (await bankEmployeeOptions.count()) ok(await bankEmployeeOptions.count() >= 3, 'bank employee selector includes at least owner and logist');
    }
    ...
    for (const name of ['target_cash_flow_center_id','target_dds_category_id','priority','counterparty_inn','purpose_contains','bank_account_id']) ok(await createModal.locator(`[name="${name}"]`).count() === 1, 'rule field ' + name);
    ...
    ok(!(await page.locator('body').innerText()).includes('Двойной клик — полный журнал'), 'obsolete double click hint removed');
    const filterOptions = page.locator('select[name="employee_ref"] option');
    ok(await filterOptions.count() >= 3, 'employee filter includes at least all + owner + logist');
    ...
    const employeeOptions = employeeCreate.locator('select[name="employee_ref"] option');
    ok(await employeeOptions.count() >= 3, 'employee selector includes at least placeholder + owner + logist');
    const employeeValues = await employeeOptions.evaluateAll(options => options.map(o => o.value));
    ok(employeeValues.some(v => v.startsWith('COMPANY_USER:')), 'company owner identity present');
    ok(employeeValues.some(v => v.startsWith('TENANT_USER:')), 'tenant user identity present');
