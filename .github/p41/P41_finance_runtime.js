...
(async () => {
  ...
  try {
    ...
    for (const label of ['Дата','Счёт','Контрагент','ИНН','Назначение','Дебет','Кредит','ЦФУ','Статья ДДС','Статус']) ok(headers.includes(label), 'bank header ' + label);
    ...
    for (const name of ['date_from','date_to','q','classification_status']) ok(await page.locator(`form.bank-controls-filter [name="${name}"]`).count() === 1, 'filter ' + name);
