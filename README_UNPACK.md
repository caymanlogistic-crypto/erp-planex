...
- `storage/` — пользовательские документы (требуют БД). Из архива исключается, но в рабочей среде НЕ удалять — иначе документы в БД станут осиротевшими, просмотр/скачивание будут возвращать 404.
- `logs/` — временные логи
