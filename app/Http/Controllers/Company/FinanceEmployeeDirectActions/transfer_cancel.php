...
verifyCsrfRequest();
$returnRef=trim((string)($_POST['return_employee_ref']??''));
try{
    ...
    if(str_starts_with($groupId,'EMPLOYEE-DIRECT-')){
        FinanceEmployeeDirectTransferEditService::cancel($pdo,$groupId,$user,(string)($_POST['reason']??''));
    }else{
        FinanceEmployeeTransferService::deleteTransfer($pdo,$groupId,$user);
    }
    $_SESSION['employee_payments_success']='Передача между сотрудниками удалена из рабочего журнала.';
