...
final class FinanceInvoiceSettlementHistoryService
{
        ...
        try {
            $stmt = $pdo->prepare(
              ...
              LEFT JOIN finance_employee_invoice_payments eip
                  ...
                  WHERE a.invoice_id=? AND a.cancelled_at IS NULL AND fo.status='POSTED'
    ...
    private static function channel(array $row): array
    {
        ...
        if ($employee !== '') return ['Сотрудник', $employee];
        ...
        if ((string)($row['money_account_type'] ?? '') === 'CASH') {
            return ['Средства компании', 'Историческая операция'];
        }
        $source = strtoupper(trim((string)($row['source'] ?? '')));
        $detail = trim((string)($row['money_account_name'] ?? ''));
        return [match ($source) {
            'BANK' => 'Расчётный счёт',
            'CASH' => 'Средства компании',
            'EMPLOYEE' => 'Сотрудник',
            default => 'Финансовая операция',
        }, preg_match('/касс/iu', $detail) ? 'Историческая операция' : $detail];
    }
