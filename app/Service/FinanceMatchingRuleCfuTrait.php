...
use PDO;
require_once __DIR__.'/FinanceMatchingRuleRegisterTrait.php';
trait FinanceMatchingRuleCfuTrait
{
 ...
 public static function saveCashFlowCenter(PDO $pdo,array $data,array $user):int{
  $id=(int)($data['id']??0);$name=trim((string)($data['name']??''));$sort=(int)($data['sort_order']??100);if($name==='')throw new \InvalidArgumentException('Название ЦФУ обязательно.');if($sort<0||$sort>100000)throw new \InvalidArgumentException('Некорректный порядок ЦФУ.');
 ...
 public static function setCashFlowCenterActive(PDO $pdo,int $id,bool $active,array $user):void{
  $check=$pdo->prepare('SELECT id FROM finance_cash_flow_centers WHERE id=?');$check->execute([$id]);if(!$check->fetchColumn())throw new \InvalidArgumentException('ЦФУ не найден.');$stmt=$pdo->prepare('UPDATE finance_cash_flow_centers SET is_active=?,updated_by_user_id=?,updated_by_role=? WHERE id=?');$stmt->execute([$active?1:0,$user['id']??null,$user['role']??null,$id]);
 }
}
