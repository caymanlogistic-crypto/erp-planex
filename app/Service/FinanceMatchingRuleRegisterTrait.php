...
trait FinanceMatchingRuleRegisterTrait{
 public static function fetchBankRegister(PDO $pdo,array $f=[]):array{
  ...
  $settlementStatuses=class_exists(FinanceSettlementStateService::class)?FinanceSettlementStateService::effectiveBankStatuses($pdo):[];
  ...
  foreach($all as $tx){$txId=(int)($tx['id']??0);if(isset($settlementStatuses[$txId]))$tx['classification_status']=$settlementStatuses[$txId];if($status!==''&&strtoupper((string)($tx['classification_status']??'UNALLOCATED'))!==$status)continue;$cfuName=$cfu[(int)($tx['cash_flow_center_id']??0)]??'';$d=$dds[(int)($tx['dds_category_id']??0)]??[];$hay=self::lower(implode(' ',[$tx['document_number']??'',$tx['counterparty_name']??'',$tx['counterparty_inn']??'',$tx['purpose']??'',$cfuName,$d['name']??'']));if($q!==''&&!str_contains($hay,$q))continue;$tx['cash_flow_center_name']=$cfuName;$tx['dds_category_name']=$d['name']??'';$tx['dds_category_code']=$d['code']??'';$rows[]=$tx;}
