...
<div class="panel">
    <div class="panel-body">
        ...
        <div class="form-section">
            ...
            <div class="tbl-wrap">
                <table class="tbl">
                    <thead><tr><th>Логист</th><th>Доступ</th><th>Тип</th><th>Выдан</th><th></th></tr></thead>
                    <tbody>
                    <?php foreach($grants as $g):
                        $isCascade = !empty($g['grant_source']) && str_starts_with($g['grant_source'] ?? '', 'contractor_cascade:');
                    ?>
                    <tr>
                        <td><?= e($g['logist_name']) ?></td>
                        <td><?= e(ui_access_level($g['access_level'] ?? null)) ?></td>
                        <td><?= $isCascade ? '<span class="badge">С каскадом</span>' : '<span class="text-muted">Только перевозчик</span>' ?></td>
                        <td class="col-muted"><?= e(ui_date($g['created_at'] ?? null)) ?></td>
                        <td class="col-actions">
                            <form method="post" action="/company/contractors/<?= $contractor['id'] ?>/unshare" class="inline-form" onsubmit="return confirm('Отозвать доступ у логиста «<?= e($g['logist_name']) ?>»? Каскадные доступы также будут отозваны.')">
                                <input type="hidden" name="granted_to_user_id" value="<?= $g['granted_to_user_id'] ?>">
            ...
            <!-- Share to one logist (quick) -->
            <form method="post" action="/company/access-grants/grant" class="grant-form" style="margin-bottom:16px;">
                ...
                <button type="submit" class="btn btn-ghost">Дать доступ (только перевозчик)</button>
            </form>

            <!-- Share to selected logists with cascade -->
            <div class="panel-section" style="margin-top:12px;">
                <h4 class="section-title">Расшарить с каскадом</h4>
                <p class="text-muted" style="margin-bottom:8px;">Выберите логистов, которым открыть доступ к перевозчику и всему рабочему контексту (экипажи, водители, транспорт).</p>
                <form method="post" action="/company/contractors/<?= $contractor['id'] ?>/share">
                    <input type="hidden" name="redirect" value="/company/contractors/<?= $contractor['id'] ?>">
                    <div style="max-height:200px; overflow-y:auto; margin-bottom:12px; border:1px solid var(--color-border, #e0e0e0); border-radius:6px; padding:8px;">
                        <?php foreach($logists as $l): ?>
                        <label style="display:flex; align-items:center; gap:8px; padding:4px 0; cursor:pointer;">
                            <input type="checkbox" name="granted_to_user_ids[]" value="<?= $l['id'] ?>" style="width:16px; height:16px;">
                            <span><?= e($l['full_name']) ?> <span class="text-muted">(<?= e($l['login']) ?>)</span></span>
                        </label>
                        <?php endforeach; ?>
                    </div>
                    <div class="form-actions" style="display:flex; gap:8px; align-items:center;">
                        <button type="submit" class="btn btn-primary">Расшарить выбранным</button>
                        <button type="submit" class="btn btn-ghost" onclick="this.form.grant_all.value='1'">Всем логистам</button>
                        <input type="hidden" name="grant_all" value="0">
                    </div>
                </form>
            </div>
        </div>
