...
document.addEventListener('DOMContentLoaded',function(){
 ...
 function initRuleForm(scope){
  const form=scope.querySelector('[data-matching-rule-form]');if(!form)return;const cfu=form.querySelector('#matching-rule-cfu'),dds=form.querySelector('#matching-rule-dds'),direction=form.querySelector('#matching-rule-direction'),cash=form.querySelector('#matching-rule-cash'),actionType=form.querySelector('select[name="action_type"]'),cashField=form.querySelector('#matching-rule-cash-field'),cashNote=form.querySelector('#matching-rule-cash-note');if(!cfu||!dds)return;let map={};try{map=JSON.parse(cfu.dataset.ddsMap||'{}')}catch(e){}
  function syncDirection(){const opt=dds.selectedOptions[0];if(!opt)return;const d=(opt.dataset.direction||'').toUpperCase();if(direction)direction.value=(d==='INCOME'||d==='EXPENSE')?d:'';if(cash){const isEmployee=!!actionType&&actionType.value==='employee_cash_settlement';const enabled=isEmployee||d==='EXPENSE';cash.disabled=!enabled;cash.required=isEmployee;if(!enabled)cash.value='';if(cashField)cashField.classList.toggle('is-disabled',!enabled);if(cashNote){if(isEmployee)cashNote.textContent='Для взаиморасчёта с сотрудником касса обязательна: списание — Банк → Касса → сотрудник, поступление — сотрудник → Касса → Банк.';else cashNote.textContent=enabled?'Для расходной статьи можно автоматически создать внутренний перевод Банк → Касса. Повторный перевод не создаётся.':'Перевод в кассу доступен только после выбора расходной статьи ДДС.';}}}
  ...
  cfu.addEventListener('change',refresh);dds.addEventListener('change',syncDirection);if(actionType)actionType.addEventListener('change',syncDirection);refresh();
 }
