...
<form method="post" action="<?= app_url('/company/finance/employee-payments/create') ?>">
 ...
 <div class="modal-body">
  <div class="form-grid two-cols">
   <div class="field"><label class="field-label">Сотрудник <span class="field-required">*</span></label><select class="field-select" name="employee_user_id" required><option value="">— Выберите сотрудника —</option><?php foreach($employees as $employee): ?><option value="<?= (int)$employee['id'] ?>"><?= e($employee['full_name']) ?></option><?php endforeach; ?></select><div class="field-note">Только активные аккаунты текущей компании.</div></div>
  ...
  <div data-source-panel="CASH">
   <div class="form-grid three-cols">
    <div class="field"><label class="field-label">Касса <span class="field-required">*</span></label><select class="field-select" name="money_account_id" required><option value="">— Выберите кассу —</option><?php foreach($cashAccounts as $account): ?><option value="<?= (int)$account['id'] ?>"><?= e($account['name']) ?> · <?= e(\App\Service\FinanceCashService::formatAmount($account['computed_balance']??'0')) ?> ₽</option><?php endforeach; ?></select></div>
