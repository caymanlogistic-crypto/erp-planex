...
| Режим | Тип | Роль |
|-------|-----|------|
| `erp-architect` | primary | Главный координатор. Общается с владельцем, ставит задачи, принимает результат. |
| `erp-uiux-designer` | subagent | UI/UX-дизайнер. Проектирует интерфейсы, готовит требования для кодера. |
| `erp-coder` | subagent | Исполнитель разработки. Пишет код строго по ТЗ. |
| `erp-qa-tester` | subagent | Тестировщик. Проверяет качество, пишет отчёт. |
