...
### 7.1 Обязательная актуализация документации после изменений

После каждой значимой модификации проекта агент обязан обновить актуальную документацию проекта. Новые важные правила, решения, workflow, ограничения и принятые изменения фиксируются в управляющих MD.

Документация обновляется точечно и только в нужных файлах. Нельзя оставлять новое правило или принятое изменение только в чате.

### 7.2 Codex Desktop как supervisor OpenCode

Codex Desktop должен экономить контекст и токены: код и массовые правки, которые можно безопасно поручить OpenCode/DeepSeek, передаются исполнителю через `tmp/deepseek-task.md`.

Codex Desktop сам отвечает за анализ задачи, постановку ТЗ, запуск OpenCode, проверку `git diff`, обязательные checks, correction prompt при ошибках, актуализацию документации и финальную приёмку.

### 7.3 Подтверждённые уроки runtime-тестирования OpenCode

После runtime-испытания зафиксированы обязательные правила:

- OpenCode эффективен для: аудит, точечные правки, runtime, DB fixtures, проверки, чистка артефактов.
- Codex не должен слепо принимать отчёты OpenCode; отчёты сначала показываются владельцу.
...
### 12.1 Branch model after 2026-07-02

Рабочая модель веток теперь фиксирована:

```text
develop = единственная ветка разработки и тестирования
master  = стабильная deploy/server ветка
```

Обязательные правила:

- Любую новую разработку, runtime-проверки и commits выполнять только в `develop`.
- `master` не менять, не коммитить, не merge/rebase/reset без отдельной прямой команды владельца.
- После завершения очередного блока сначала commit в `develop`, и только потом владелец отдельно решает, нужно ли синхронизировать `master`.
- При проверках branch/head сначала убедиться, что агент действительно находится в `develop`.
