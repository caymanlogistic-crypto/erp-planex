...
STATUS: ACCESS_REGRESSION_FIXED

Задача переработки меню и UX блока подрядчиков/перевозчиков выполнена в 6 блоках. Обнаружена и исправлена runtime-регрессия доступа логиста: пустой список показывал «Нет доступа» вместо empty state. Исправлено в 4 view-файлах (contractors, driver_vehicle_blocks, vehicles, drivers).
