...
30. CREATE_WITH_DOCUMENTS: при создании водителя/транспорта/перевозчика документы (multiple files) загружаются в той же форме. Файлы обрабатываются ПОСЛЕ успешного INSERT сущности, ДО выставления $success. entity_type жёстко задан в обработчике, entity_id = lastInsertId(). Стратегия ошибок: Вариант Б — валидные сохраняются, невалидные пропускаются с сообщением. Orphan-файлы удаляются при ошибке metadata.
31. WEBP: формат `.webp` (MIME `image/webp`) добавлен в whitelist расширений во всех точках валидации документов (upload, replace, create-формы). Whitelist не ослаблен — webp разрешён только как изображение.
