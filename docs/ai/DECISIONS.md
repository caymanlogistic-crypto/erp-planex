...
92. **(SUPERSEDED by #93) DB pool one-way consumption usage journal (2026-07-05).** Replaced by reusable pool tracking (#93).

93. **DB pool reusable tracking (2026-07-06).** Pool DBs are reusable, not one-way consumed. On company deletion, the pool DB is fully cleaned (`cleanCompanyPoolDatabase()` — drops all tables) and released (`releasePoolDb()` — sets `released_at = NOW()`). After release, the pool DB becomes free for the next creation. `findFreePoolDb()` considers a DB busy only if assigned to an active company (`companies.db_identifier`) OR reserved with `released_at IS NULL` in `company_db_pool_usage`. Released rows do NOT block reuse. `markPoolDbUsed()` reuses existing released rows if available, or throws if the DB is already reserved for another company. This makes pool DBs recyclable without manual DBA intervention. See `app/Support/company_database.php` and `app/Http/Routes/superadmin_company_delete.php`. Documentation updated in `CURRENT_TASK.md`, `DECISIONS.md`, `HANDOFF_FOR_NEW_CHAT.md`.

94. **DB pool release only after successful cleanup (2026-07-06).** `releasePoolDb()` must NOT run if `cleanCompanyPoolDatabase()` failed. This prevents dirty DBs (with leftover data) from being marked free (`released_at = NOW()`) and reused by the next company. The delete report records `pool_db_release` as `skipped` with reason `cleanup_failed` when cleanup fails. Additionally, `cleanCompanyPoolDatabase()` now drops triggers, views, routines (procedures/functions), and events — not just base tables. A `companyDbQuoteIdentifier()` helper for safe backtick escaping was added. `FOREIGN_KEY_CHECKS` is restored to `1` in a `finally` block even if an exception occurs. See `app/Support/company_database.php` and `app/Http/Routes/superadmin_company_delete.php`.
