...
31. Документы могут загружаться inline при создании сущностей (водитель, транспорт, перевозчик). Предопределённые типы документов сеются миграциями с category='predefined'. Пользовательские типы создаются через /company/document-types или inline через форму создания.
