...
SUPERADMIN-админка закрыта как полноценный центр управления всеми экспедиторами, Руководителями и пользователями компаний.

Реализовано:
- **Расширенный реестр компаний** (10 колонок: ID, Название, ИНН, Статус, Provisioning, Локальная БД, Руководитель, Пользователей, Создан, Действия) с row actions (V/E/O/U + статусные кнопки)
- **Расширенная карточка компании** с секциями: Пользователи, Справочники, Документы, Доступы, Действия
- **Управление статусами компаний**: activate/block/archive (POST, confirm, без destructive операций)
- **Управление пользователями компании** (объединённая таблица Руководитель + Логисты с row actions)
- **Управление локальными логистами из SUPERADMIN**: карточка, редактирование, сброс пароля, активация/блокировка/архивирование
- **Мониторинг справочников** (counts: clients, contractors, drivers, vehicles, crews)
- **Мониторинг документов** (таблица с download)
- **Мониторинг доступов** (entity_access_grants, REVOKE DEFERRED)

Правило безопасности паролей зафиксировано:
...
Маршруты: 27 SUPERADMIN маршрутов (13 существующих + 14 новых).
QA: SUPERADMIN_MANAGEMENT_ACCEPTED (65/65 PASS, 0 BLOCKER).

### Причина

Владелец выявил, что SUPERADMIN-блок не закрыт: не хватает управления пользователями компаний, статусами, мониторинга справочников/документов/доступов.
