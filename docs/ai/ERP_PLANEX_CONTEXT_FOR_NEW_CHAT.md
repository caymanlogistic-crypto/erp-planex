...
- `erp-architect` (primary, default_agent) — главный координатор, общается с владельцем, решает маршрут задачи, подключает дизайнера/кодера/QA, принимает результат или запускает второй круг;
- `erp-uiux-designer` (primary/selectable) — UI/UX-дизайнер, проектирует интерфейсы и обязан создавать/обновлять MD-шаблоны страниц в `docs/ui/pages/`;
- `erp-coder` (primary/selectable) — исполнитель разработки PHP/MySQL/HTML/CSS/JS, работает только по ТЗ архитектора, архитектурным MD и UI-шаблону страницы, если задача затрагивает интерфейс;
- `erp-qa-tester` (primary/selectable) — тестировщик, проверяет соответствие ТЗ, UI-шаблону, архитектуре, runtime, безопасности, документации и логам.
...
Для UI-задач действует обязательный workflow: `erp-architect` определяет, нужен ли дизайнер; `erp-uiux-designer` создаёт или обновляет MD-шаблон страницы в `docs/ui/pages/`; `erp-coder` реализует строго по этому шаблону; `erp-qa-tester` проверяет соответствие шаблону; `erp-architect` принимает результат или запускает второй круг.

Для задач без интерфейса архитектор может передавать задачу сразу кодеру, затем QA. Если есть сомнение, затрагивает ли задача UI, дизайнер подключается обязательно.

Кодер работает по принципам Karpathy-style engineering: think before coding, simplicity first, surgical changes, goal-driven/verifiable execution. Нельзя расширять scope, переписывать лишнее, придумывать бизнес-логику или сдавать работу без проверок.

QA возвращает один из статусов: `ACCEPTED`, `NEEDS_REWORK`, `REJECTED`, `BLOCKED`. Если есть blocking issue, отсутствие UI-шаблона для UI-задачи, не выполнен `php -l` после PHP-изменений или не обновлены нужные MD/логи, статус не может быть `ACCEPTED`.
...
- `docs/ui/DESIGN_CODE_INTEGRATION.md` — главный UI/UX и дизайн-код регламент.
- `docs/ui/pages/` — MD-шаблоны страниц, создаваемые дизайнером.
...
3. Перед созданием/изменением бизнес-страниц использовать MD-шаблоны `docs/ui/pages/` от дизайнера.
4. После этого перейти к техническому коду: PDO-обёртка и роутер.
5. После отдельного задания начать SUPERADMIN.
...
### Актуальные правила агентов

- `erp-architect` обязан перед каждой задачей определить маршрут: дизайнер нужен / дизайнер не нужен / кодер / QA. UI-задача не уходит кодеру без актуального MD-шаблона страницы.
- `erp-uiux-designer` обязан при создании или изменении страницы создавать/обновлять `docs/ui/pages/[page-name].md`; этот файл является источником истины для кодера и QA.
- `erp-coder` обязан работать точечно и проверяемо: читать MD-контекст, соблюдать `MODULE_PATTERN.md` и `PHP_APP_SKELETON.md`, не писать UI без шаблона, выполнять `php -l` для изменённых PHP-файлов и обновлять логи/MD.
- `erp-qa-tester` обязан проверять ТЗ, UI-шаблон, архитектуру, runtime, security/secrets, документацию и логи; без этих проверок задача не принимается.



### UI/UX page-template workflow

Для любой UI-задачи дизайнер создаёт или обновляет MD-шаблон страницы:

```text
docs/ui/pages/[page-name].md
```

Шаблон страницы является источником истины для кодера и QA.

Кодер не должен сам придумывать layout, состав таблиц, формы, состояния, классы и UX-поведение.

QA проверяет реализацию по актуальному MD-шаблону страницы.
...
2026-06-11 — ChatGPT: обновлены правила UI/UX-дизайнера и агентский workflow для UI-страниц. Закреплено, что дизайнер создаёт/обновляет MD-шаблоны страниц в `docs/ui/pages/`; кодер реализует строго по шаблону; QA проверяет по шаблону; архитектор принимает или запускает второй круг. Добавлено DECISION-0017.


2026-06-11 — ChatGPT: проведена чистка документации `docs`. Удалены дублирующие/мусорные UI-файлы: `docs/ui/UI_UX_RULES.md` и дубликат `docs/ui/ui/DESIGN_CODE_INTEGRATION.md`. Главный UI-регламент теперь только `docs/ui/DESIGN_CODE_INTEGRATION.md`; шаблоны страниц хранятся в `docs/ui/pages/`.
