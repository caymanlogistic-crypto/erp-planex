...
Проект готов к следующему техническому этапу перед SUPERADMIN.

Ближайшие рекомендуемые технические шаги:
1. Создать PDO-обёртку для подключения к БД.
2. Создать простой роутер.
3. После отдельного задания начать миграции центральной БД SUPERADMIN.
4. После отдельного задания начать реализацию SUPERADMIN.
