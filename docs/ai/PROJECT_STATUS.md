...
PDO-обёртка и роутер созданы и проверены (2026-06-11). Техническое ядро готово: Database PDO wrapper, простой GET-роутер с параметрами, интеграция в точку входа. Следующий шаг: SUPERADMIN. Workflow MD-шаблонов страниц закреплён и готов к использованию.
...
2026-06-11 23:50 — erp-architect: приёмка PDO-обёртки и роутера через agent workflow (coder → QA → architect). QA ACCEPTED. Баг `use ($config)` исправлен архитектором. Техническое ядро готово. Следующий шаг: SUPERADMIN. Ранее: 2026-06-11 23:45 — PDO-обёртка и роутер созданы кодером.
