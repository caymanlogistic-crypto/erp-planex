...
Перед кодингом реестра компаний нужно обновить схему центральной БД под новые требования:
- главный пользователь экспедитора хранится в центральной БД;
- главный пользователь должен быть явно связан с одной компанией / экспедитором;
- создание главного пользователя является отдельным действием после создания экспедитора.

Точную схему связи нужно утвердить отдельной задачей. Не выбирать произвольно между `company_id` в таблице пользователей и отдельной link-таблицей без архитектурного решения.
