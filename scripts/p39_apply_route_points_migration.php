...
if ($sql === false || trim($sql) === '') {
    throw new RuntimeException('P39 migration SQL is missing or empty.');
}
...
foreach ($companies as $company) {
    ...
    if ((int) $lock->fetchColumn() !== 1) {
        throw new RuntimeException('Cannot acquire P39 migration lock.');
    }
    ...
    try {
        ...
        if ($stored !== false) {
            if (!hash_equals((string) $stored, $checksum)) {
                throw new RuntimeException('P39 migration checksum mismatch in company ' . (int) ($company['id'] ?? 0));
            }
            $existingCount++;
        } else {
        ...
        $tableCheck = $local->query(
            "SELECT COUNT(*) FROM information_schema.TABLES
              WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'linear_route_points'"
        );
        if ((int) $tableCheck->fetchColumn() !== 1) {
            throw new RuntimeException('linear_route_points table missing after P39 migration.');
        }
    } finally {
...
printf(
    "P39_MIGRATION_OK companies=%d applied=%d existing=%d checksum=%s\n",
    $count,
    $appliedCount,
    $existingCount,
    $checksum
