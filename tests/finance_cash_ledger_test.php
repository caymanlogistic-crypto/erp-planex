...
$pdo->exec("CREATE TABLE finance_operations (
    ...
    comment TEXT NULL, classification_rule_id INT UNSIGNED NULL, created_at DATETIME NOT NULL
) ENGINE=InnoDB");
$pdo->exec("CREATE TABLE finance_employee_movements (
    id INT UNSIGNED PRIMARY KEY,
    finance_operation_id INT UNSIGNED NOT NULL,
    employee_identity_type VARCHAR(20) NULL,
...
$pdo->exec("INSERT INTO finance_operations
    (id,operation_type,status,source,money_account_id,transfer_account_id,transfer_group_id,transfer_direction,operation_date,amount,currency,purpose,comment,classification_rule_id,created_at)
VALUES
    (10,'TRANSFER','POSTED','TRANSFER',1,2,'TRF_BANK_CASH','out','2026-08-13',549.97,'RUR','Покупка 13.08.2026',NULL,NULL,'2026-08-13 10:00:00'),
    (11,'TRANSFER','POSTED','TRANSFER',2,1,'TRF_BANK_CASH','in','2026-08-13',549.97,'RUR','Покупка 13.08.2026','Банк → касса',NULL,'2026-08-13 10:00:01'),
    (12,'EXPENSE','POSTED','CASH',2,NULL,NULL,NULL,'2026-08-14',5000.00,'RUR','Выдано под отчёт',NULL,NULL,'2026-08-14 09:00:00'),
    (13,'INCOME','POSTED','CASH',2,NULL,NULL,NULL,'2026-08-14',1250.00,'RUR','Возврат подотчёта',NULL,NULL,'2026-08-14 10:00:00'),
    (14,'TRANSFER','POSTED','TRANSFER',2,1,'TRF_CASH_BANK','out','2026-08-15',700.00,'RUR','Возврат на расчётный счёт',NULL,NULL,'2026-08-15 11:00:00'),
    (15,'EXPENSE','POSTED','CASH',2,NULL,NULL,NULL,'2026-08-15',549.97,'RUR','Передано сотруднику: Иванов Иван Иванович · Покупка 13.08.2026','Старое дублирующее списание',NULL,'2026-08-15 14:00:00'),
    (16,'INCOME','POSTED','CASH',2,NULL,NULL,NULL,'2026-08-15',99.00,'RUR','Неразнесённый источник',NULL,NULL,'2026-08-15 15:00:00'),
    (17,'INCOME','POSTED','CASH',2,NULL,NULL,NULL,'2026-08-16',2000.00,'RUR','Возврат сотрудником',NULL,NULL,'2026-08-16 09:00:00'),
    (18,'TRANSFER','POSTED','TRANSFER',2,1,'TRF_EMP_RETURN','out','2026-08-16',2000.00,'RUR','Внесение наличных',NULL,NULL,'2026-08-16 09:00:01')");
$pdo->exec("INSERT INTO finance_employee_movements
    (id,finance_operation_id,employee_identity_type,employee_identity_id,employee_name_snapshot,movement_type,bank_transaction_id) VALUES
    (1,12,NULL,NULL,'Петров Пётр Петрович','PAYMENT',NULL),
    (2,13,NULL,NULL,'Иванов Иван Иванович','RETURN',NULL),
    (3,15,NULL,NULL,'Иванов Иван Иванович','PAYMENT',NULL),
    (4,17,NULL,NULL,'Спугов Владимир Владимирович','RETURN',NULL)");
...
assertCashLedger(($byId[11]['source_label'] ?? '') === '40702810000000000001', 'bank source is shown as account number only');
...
assertCashLedger(($byId[11]['handoff_date'] ?? '') === '', 'compact cash journal does not append handoff date to employee');
...
assertCashLedger(($byId[17]['handoff_recipient_label'] ?? '') === '40702810000000000001', 'reverse lifecycle shows only destination bank account number');
...
assertCashLedger(($byId[14]['source_label'] ?? '') === '40702810000000000001', 'cash transfer counterpart is shown as account number only');
